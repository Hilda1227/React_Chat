...
module.exports = {
  ...
  async newMessage (info, socket, cb, io) {
    ...
    // 如果私聊
    if (type === 'private') {
      const target = await User.findOne({ _id: toId }).populate('socket', 'socket_id')
      // 如果已被对方屏蔽， 则返回
      // if(Array.from(target.shield).indexOf(user_id)) return cb({ isError: false, msg: "您已被对方屏蔽" });
      const newMsg = await new PrivateMsg({ from: from._id, to: target._id, content, msgType, createAt: Date.now() })
      // 如果对方在线
      ...
      const group = await Group.findOne({ _id: toId })
      // 如果已被禁言
      if (!Array.from(group.block).every(item => String(item) !== user_id)) { return cb({ isError: true, msg: '您已被禁言' }) }
      const newMsg = await new GroupMsg({ content, from: user_id, to: toId, msgType, createAt: Date.now() }).save()
