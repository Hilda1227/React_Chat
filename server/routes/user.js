...
router.post('/upload/file', async function (ctx) {
  ...
  try {
    const ret = await uploadFile(`${+new Date() + file.name}`, file.path)
...
router.post('/api/createGroup', async function (ctx) {
  ...
  const info = { nickname, creator: _id }
  const file = ctx.request.files[0]
  if (file) {
    try { var ret = await uploadFile(`${Date.now() + file.name}`, file.path) } catch (err) { return ctx.body = ({ isError: true, msg: '头像上传失败' }) }
    info.avatar = ret.src
  }
...
router.post('/api/modifyUserInfo', async function (ctx) {
  ...
  const exist = await User.findOne({ nickname, _id: { $ne: _id } })
  if (exist) return ctx.body = ({ isError: true, msg: '该用户名已被使用' })
  const info = { nickname, sex, place }
  const file = ctx.request.files[0]
  if (file) {
    try { var ret = await uploadFile(`${Date.now() + file.name}`, file.path) } catch (err) { return ctx.body = ({ isError: true, msg: '发送失败' }) }
    info.avatar = ret.src
  }
