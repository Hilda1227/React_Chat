...
function formData (data) {
  ...
  Object.keys(data).forEach(key => {
    if (typeof data[key] !== 'null' && typeof data[key] !== 'undefined') { console.log(data[key]) }
...
export const createGroup = (info) => {
  const formdata = formData(info)
  return axios.post('/api/createGroup', formdata, {
    headers: { 'Content-Type': 'multipart/form-data' }
  })
    .then(res => {
      console.log('返回', res.data)
      if (!res.data.isError) { return dispatchAction(initRoomList(info._id)) }
